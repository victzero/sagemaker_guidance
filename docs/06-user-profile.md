...
| SingleSignOnUserValue | IAM User ARN | arn:aws:iam::123456:user/sm-rc-alice |
...
在 IAM 认证模式下，User Profile 通过 **SingleSignOnUserValue** 与 IAM User 绑定：

```
绑定方式:
SingleSignOnUserValue: arn:aws:iam::{account-id}:user/{iam-user-name}

示例:
- UserProfileName: profile-rc-alice
- SingleSignOnUserValue: arn:aws:iam::123456789012:user/sm-rc-alice
```
...
- SingleSignOnUserValue: arn:aws:iam::{account}:user/sm-{team}-{name}
- Tags:
    - Key: Team, Value: {team}
